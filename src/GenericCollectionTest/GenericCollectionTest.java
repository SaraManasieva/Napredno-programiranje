...
{
    private Map<String,Set<T>> mapaNaElementiOdTipTpoKategorija;

    public GenericCollection() {
        this.mapaNaElementiOdTipTpoKategorija=new HashMap<>();
    }

    public void addGenericItem(String category, T element) {
        //метод за додавање на нов елемент во дадена категорија
        this.mapaNaElementiOdTipTpoKategorija.putIfAbsent(category,new TreeSet<T>());
        //this.mapaNaElementiOdTipTpoKategorija.get(category).add(element);
        this.mapaNaElementiOdTipTpoKategorija.computeIfPresent(category,(k,v)->{
            v.add(element);
            return v;
        });
    }
    ...
    Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to)
    {
        ...
        return this.mapaNaElementiOdTipTpoKategorija.values().stream()
                .flatMap(mnozestvoOdElementiOdTipT->mnozestvoOdElementiOdTipT.stream())
    ...
    Collection<T> itemsFromCategories (List<String> categories)
    {
        ...
        return this.mapaNaElementiOdTipTpoKategorija.keySet()
                .stream().filter(str->categories.contains(str))
                .map(str->mapaNaElementiOdTipTpoKategorija.get(str))
                .flatMap(m->m.stream())
                .collect(Collectors.toCollection(() -> new TreeSet<T>(Comparator.reverseOrder())));
    }
    ...
    public Map<String, Set<T>> byMonthAndDay()
    {
        ...
        return this.mapaNaElementiOdTipTpoKategorija.values().stream()
                .flatMap(mnozestvoOdElementiOdT->mnozestvoOdElementiOdT.stream())
                .collect(Collectors.groupingBy(t->String.format("%02d-%02d",t.getTimestamp().getMonthValue(),t.getTimestamp().getDayOfMonth()),TreeMap::new,
                        Collectors.toCollection(() -> new TreeSet<T>(Comparator.reverseOrder()))));
    }

    public Map<Integer, Long> countByYear()
    {
        //враќа мапа во која што клучеви се сите години кога има креирани некој елелемт, а соодветната вредност е бројот на елементи креирани во таа година.
        return this.mapaNaElementiOdTipTpoKategorija.values().stream()
                .flatMap(mnozestvo->mnozestvo.stream())
                .collect(Collectors.groupingBy(t->t.getTimestamp().getYear(),TreeMap::new,Collectors.counting()));
    }
}
