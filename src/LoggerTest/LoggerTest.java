...
class LogSystem<T extends ILog & Comparable<T>> {
    ...
    public LogSystem(ArrayList<T> logs) {
        this.logsList = logs.stream().collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = logsList->this.logsList.stream().filter(log->log.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));

        //Процесор којшто ќе ги врати само логовите чиишто пораки се пократки од 100 карактери
        LogProcessor<T> secondLogProcessor = logsList->this.logsList.stream().filter(log->log.getMessage().length()<100).collect(Collectors.toCollection(ArrayList::new));

        //Процесор којшто ќе ги врати логовите сортирани според временскиот печат во растечки редослед (од најстар кон најнов лог).
        LogProcessor<T> thirdLogProcessor = logsList->this.logsList.stream().sorted(Comparator.comparing(ILog::getTimestamp)).collect(Collectors.toCollection(ArrayList::new));
