package GenericCollectionTest_zad8;
...
{
    private Map<String,Set<T>>mapaNaTPoKategorija;

    public GenericCollection() {
        this.mapaNaTPoKategorija=new TreeMap<>();
    }

    void addGenericItem (String category, T element)
    {
        this.mapaNaTPoKategorija.putIfAbsent(category,new TreeSet<>());
        this.mapaNaTPoKategorija.get(category).add(element);
    }
    Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to)
    {
        return this.mapaNaTPoKategorija.values().stream()
    ...
    {
        return this.mapaNaTPoKategorija.keySet()
                .stream().filter(c->categories.contains(c))
                .flatMap(c->mapaNaTPoKategorija.get(c).stream())
                .collect(Collectors.toCollection(() -> new TreeSet<T>(Comparator.reverseOrder())));
    }
