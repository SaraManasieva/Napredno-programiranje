...
class Scheduler<T>
{
    private TreeMap<Date,T> mapaOdTPoDate;

    public Scheduler() {
        this.mapaOdTPoDate=new TreeMap<>();
    }
    void add(Date d, T t)
    {
        this.mapaOdTPoDate.put(d,t);
    }

    boolean remove(Date d)
    {
        return this.mapaOdTPoDate.remove(d,this.mapaOdTPoDate.get(d));
    }
    T next()
    {
        return this.mapaOdTPoDate.higherEntry(new Date()).getValue();
    }
    T last()
    {
        return this.mapaOdTPoDate.floorEntry(new Date()).getValue();
    }

    ArrayList<T>getAll(Date begin,Date end)
    {
        return new ArrayList<T>(this.mapaOdTPoDate.subMap(begin,end).values());
    }

    T getFirst()
    {
        return this.mapaOdTPoDate.firstEntry().getValue();
    }

    T getLast()
    {
        return this.mapaOdTPoDate.lastEntry().getValue();
    }
