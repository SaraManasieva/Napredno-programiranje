...
class Schedulers {
    public static <T extends Task> TaskScheduler<T> getOrdered() {
        return new TaskScheduler<T>() {
            @Override
            public List<T> schedule(T[] nizaOdZad) {
                return Arrays.stream(nizaOdZad).sorted(Comparator.comparing(Task::getOrder))
    ...
    public static <T extends Task> TaskScheduler<T> getFiltered(int order) {
        ...
        return new TaskScheduler<T>() {
            @Override
            public List<T> schedule(T[] nizaOdZad) {
                return Arrays.stream(nizaOdZad).filter(task->task.getOrder()<order).collect(Collectors.toList());
            }
        };
