package GenericMapTest;

import java.util.*;
import java.util.stream.Collectors;
import java.util.stream.IntStream;

interface MergeStrategy<K,V>
{
     V strategijaZaSpojuvanjeZaIstiKlucevi(V v1,V v2);
}
class MapOps
{
    public static <K extends Comparable<K>,V> Map<K,V> merge(Map<K,V> prva,Map<K,V> vtora,MergeStrategy<K,V> mergeStrategy)
    {
        Map<K,V> rezultantna=new TreeMap<>(prva);
        vtora.forEach((k,v)->rezultantna.merge(k,v, (v1, v2) -> mergeStrategy.strategijaZaSpojuvanjeZaIstiKlucevi(v1, v2)));
        return rezultantna;
    }
}
...
public class GenericMapTest {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Mergeable integers
            ...
            MergeStrategy<Integer,Integer> mergeStrategy = (v1,v2)->v1+v2;
            ...
            MergeStrategy<String,String> mergeStrategy = (v1,v2)->v1+v2;
            ...
            MergeStrategy<Integer,Integer> mergeStrategy = (v1,v2)->Math.max(v1,v2);
            ...
            MergeStrategy<String,String> mergeStrategy = (v1,v2)->v1.replace(v2,IntStream.range(0,v2.length()).mapToObj(i->"*").collect(Collectors.joining("")));
            printMap(MapOps.merge(mapLeft, mapRight, mergeStrategy));
        }
    }
