...
class DoubleMatrix
{
    ...
    DoubleMatrix(double a[], int m, int n) throws InsufficientElementsException {
        ...
        int brojNaSkip=a.length-(m*n);
        for(int i=0;i<m;i++)
        {
            for(int j=0;j<n;j++)
            {
                matrica[i][j]=a[brojNaSkip];
                brojNaSkip=brojNaSkip+1;
            }
        }
    }
    ...
    {
        //[m x n]
        return String.format("[%d x %d]",matrica.length,matrica[0].length);
    }
    ...
    double maxElementAtRow(int row) throws InvalidRowNumberException {
        ...
        return Arrays.stream(matrica[row-1]).max().orElse(0);
    }
    ...
    double maxElementAtColumn(int column) throws InvalidColumnNumberException {
        ...
        return IntStream.range(0,rows()).mapToDouble(i->matrica[i][column-1])
                .max().orElse(0);
    }

    double sum()
    {
        return IntStream.range(0,rows()).mapToDouble(i->IntStream.range(0,columns()).mapToDouble(j->matrica[i][j]).sum()).sum();
    }

    double[]toSortedArray()
    {
        double []sortirana=Arrays.stream(matrica).flatMapToDouble(redica-> Arrays.stream(redica)).sorted().toArray();
        return IntStream.range(0,sortirana.length).mapToDouble(i->sortirana[sortirana.length-1-i]).toArray();
    }
    ...
    public String toString() {
        return Arrays.stream(matrica).map(red->Arrays.stream(red).mapToObj(el->String.format("%.2f",el)).collect(Collectors.joining("\t")))
