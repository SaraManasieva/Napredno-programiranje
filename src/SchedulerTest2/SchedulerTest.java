...
class Timestamp<T>implements Comparable<Timestamp<? extends T>>
{
    ...
    public String toString() {
        return String.format("%s %s",localDateTime,t);
    }

    @Override
    public int compareTo(Timestamp<? extends T> o) {
        Comparator<Timestamp<? extends T>>comparator=Comparator.comparing(Timestamp::getTime);
...
class Scheduler<T>
{
    ...
    Timestamp<T> next()
    {
        return this.timestamps.stream().filter(t->t.getTime().isAfter(LocalDateTime.now()))
                .sorted(Comparator.comparing(Timestamp::getTime))
                .findFirst().orElse(null);
    }
    Timestamp<T>last()
    {
        return this.timestamps.stream().filter(t->t.getTime().isBefore(LocalDateTime.now()))
    ...
    {
        return this.timestamps.stream().filter(t->t.getTime().isAfter(begin)&&t.getTime().isBefore(end))
